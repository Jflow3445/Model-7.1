    ...
    def forward(
        ...
        if closed_trades:
            for t in closed_trades:
                ...
                # Optionally integrate costs here IF they are not already included in the trade dict.
                # Default is False to avoid double-charging (your env already subtracts costs).
                if self.integrate_costs_in_reward and ("slippage" not in t and "commission" not in t):
                    c_slip = _safe_float(t.get("slippage", self.slippage_per_unit))
                    c_comm = _safe_float(t.get("commission", self.commission_per_trade))
                    pnl_adj = pnl_raw - (c_slip + c_comm) * abs(vol)
                else:
                    pnl_adj = pnl_raw

                # Use adjusted pnl to compute R
                t_for_r = dict(t)
                t_for_r["pnl"] = pnl_adj
                ...
                if pnl_adj > 0:
                    gross_profit_step += pnl_adj
                elif pnl_adj < 0:
                    gross_loss_step += -pnl_adj  # absolute
        ...
        if open_trades:
            for t in open_trades:
                ...
                if sl <= 0.0 or entry <= 0.0 or side == 0:
                    exposure_R += 2.0 * abs(vol)  # no SL → count as 2R to force discipline
                else:
        ...
        if self.smoothing_alpha > 0.0:
            ...
            out_val = self.smoothing_alpha * total + (1.0 - self.smoothing_alpha) * prev
            self.previous_reward.copy_(_to_device(torch.tensor(out_val, dtype=torch.float32), device))

        return torch.tensor(out_val, dtype=torch.float32, device=device)
    # --------------------------
    # Lifecycle
    # --------------------------
    def reset(self) -> None:
