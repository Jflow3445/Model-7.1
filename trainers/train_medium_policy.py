...
from models.medium_policy import (
    MediumTermOHLCPolicy,
)
...
def load_broker_meta(json_path: Path) -> Dict[str, Dict]:
    if json_path.exists():
        with json_path.open("r", encoding="utf-8") as f:
            return json.load(f)
    logging.warning(f"[train_medium] broker_stops.json not found at {json_path}, using zeros.")
    return {}
...
class CheckpointAndRcloneCallback(BaseCallback):
    ...
    def __init__(self, checkpoint_freq: int, ckpt_dir: str,
                 name_prefix: str = "medium_policy_ckpt",
...
# ──────────────────────────────────────────────────────────────────────────────
class MediumBacktestEnv(gym.Env):
    """
    ...
    def __init__(
        ...
        risk_budget = max(4.0, 0.5 * self.n_assets)
        ...
        self.reward_fn = RewardFunction(
            ...
            inactivity_weight=0.0015,
            inactivity_grace_steps=24,          # ~24 hours grace
            holding_threshold_steps=48,         # start nudging after 48 hours in trade
...
# VecEnv factory
# ──────────────────────────────────────────────────────────────────────────────
def make_medium_env(rank: int, seed: int, window: int, symbols=None) -> Callable[[], gym.Env]:
    def _init():
...
def get_latest_checkpoint(ckpt_dir: str, last_ckpt_path: str, main_save_path: str) -> Optional[str]:
    ckpt_files = glob.glob(os.path.join(ckpt_dir, "medium_policy_ckpt_*_steps.zip"))
    if ckpt_files:
...
def train_medium_policy(
    ...
    algo_cls = PPO
    policy_cls = MediumTermOHLCPolicy
    algo_kwargs = dict(
        n_steps=n_steps,
        ...
        vf_coef=0.5,
        max_grad_norm=0.5,
        tensorboard_log=os.path.join(LOGS_DIR, "tb_medium_policy"),
        device="cuda",
    )
    policy_kwargs = dict(window=window)

    os.makedirs(MODELS_DIR, exist_ok=True)
    ckpt_dir = os.path.join(MODELS_DIR, "checkpoints_medium")
    os.makedirs(ckpt_dir, exist_ok=True)
    main_save_path = os.path.join(ckpt_dir, "medium_policy.zip")
    last_ckpt_path = os.path.join(ckpt_dir, "medium_policy_last.zip")
