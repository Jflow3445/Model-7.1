...
# ──────────────────────────────────────────────────────────────────────────────
# Shared utils/blocks (mirrors medium policy upgrades)
# ──────────────────────────────────────────────────────────────────────────────
...
class CrossAssetAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        dtype_in = x.dtype
        B = x.size(0)
        max_rows = int(os.environ.get("MEDIUM_XATTN_MAX_ROWS", "4096"))
        outs = []
...
class MediumTermFeatureExtractor(BaseFeaturesExtractor):
    ...
    def __init__(
        ...
        **_: Any,
        ...
        self.max_tokens_per_chunk = int(os.environ.get("MEDIUM_MAX_TOKENS", "120000"))
    ...
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        # [B, N, per_asset], then slice bars -> [B, N, W, 5]
        obs = obs.reshape(B, self.n_assets, self.per_asset)
        bars5_flat = obs[:, :, : 5 * self.window]                                  # [B, N, 5W]
        bars = bars5_flat.view(B, self.n_assets, 5, self.window).transpose(2, 3)   # [B, N, W, 5]
        bars = bars.contiguous()
        ...
        max_rows_cap = int(os.environ.get("MEDIUM_MAX_ROWS", "8192"))  # default igher to reduce overhead
